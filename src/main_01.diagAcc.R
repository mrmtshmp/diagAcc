...
load(
  file = sprintf('%s/%s', dir.data, fn.ADS)
  )
  ...
  rpart(
    imp.data %>%
      mutate(
        com_comp.cancer = 
          as.numeric(
            com.solid_tum + com.hem_malig >0
            ), # c.p.m,2020/07/10
        
        com_comp.auto_imn = 
          as.numeric(
            com.RA + com.auto_imn >0
            ) # c.p.m,2020/07/10
        ),
...
res.rpart.var.test_commed <- 
  rpart(
    imp.data %>%
      mutate(
        com_comp.cancer = as.numeric(com.solid_tum+com.hem_malig >0), # c.p.m,2020/07/10
        com_comp.auto_imn = as.numeric(com.RA + com.auto_imn >0) # c.p.m,2020/07/10
        ),
...
res.rpart.cancer_all.var.test <- 
  rpart(
    imp.data %>%
      ...
      dplyr::filter(com_comp.cancer==1),
    formula = as.formula(fml.string_var.test)
  )
...
res.rpart.auto_imn_all.var.test <- 
  rpart(
    imp.data %>%
      ...
      dplyr::filter(com_comp.auto_imn==1),
    formula = as.formula(fml.string_var.test)
  )
...
pdf(sprintf('%s/%s',dir.output,'rpart.total_subj.pdf'))
...
pdf(sprintf('%s/%s',dir.output,'rpart.cancer_all.pdf'))
...
pdf(sprintf('%s/%s',dir.output,'rpart.auto_imn_all.pdf'))
...
factor.med  <- colnames(imp.data)[grep("^med",colnames(imp.data))]
factor.com  <- colnames(imp.data)[grep("^com",colnames(imp.data))]
...
df.itt.simul_mf.find_select_bias <-
  data.frame(id=1:n.itt, itt=n.itt)
...
res_imp_maxBias.itt.simul_mf.find_select_bias <- res.itt.simul_mf.find_select_bias %>% 
  ddply(
    .variables = .(factor),
    .fun = function(D){
      D.perm <- D[2:nrow(D),]
...
ggdata.ecdf <- 
  ecdf.res_imp_maxBias.itt.simul_mf.find_select_bias %>% 
  ggplot(aes(x = log(Bias), y = ecdf))

quartz(type = "pdf", width = 42,height = 21,
  file =
    sprintf(
      "%s/%s", dir.output,"ecdf.ratio_of_OR.pdf"
      )
  )
plot(
  ggdata.ecdf +
...
write.csv(
  ecdf.res_imp_maxBias.itt.simul_mf.find_select_bias[ecdf.res_imp_maxBias.itt.simul_mf.find_select_bias$id==1,],
  file = sprintf("%s/%s", dir.output,"ecdf.ratio_of_OR.csv")
  )
...
# Confidence intervals of accuracy metrics --------------------------------
#' c.p.m 2020/07/10 This section was created at this modification. 

ci.coords(roc=roc.var.y.var.test,x='best')
ci.coords(roc=roc.var.y.var.test_med.MTX,x='best')
ci.coords(roc=roc.var.y.var.test_med.PSLgte.5,x='best')
ci.coords(roc=roc.var.y.var.test_med.MTX,x='best')
